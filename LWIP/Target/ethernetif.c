...
/* USER CODE END PHY_PRE_CONFIG */
  ...
  /* Initialize the USER_PHY ETH PHY */
...
void HAL_ETH_MspInit(ETH_HandleTypeDef* ethHandle)
{
  ...
  /* USER CODE BEGIN ETH_MspInit 1 */
    ...
    /* ETH callbacks release FreeRTOS semaphores: priority 5 is safe for
       FromISR APIs with configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY == 5. */
...
/* USER CODE BEGIN HAL ETH RxLinkCallback */
  ...
  *ppEnd  = p;
  ...
  /*
   * RX buffers are written by ETH DMA.  Cache maintenance is needed only
   * when the D-Cache is enabled.  Align the whole range to cache lines:
   * SCB_InvalidateDCache_by_Addr() operates on 32-byte granularity.
   */
  if ((SCB->CCR & SCB_CCR_DC_Msk) != 0U)
  {
    uint32_t cache_start = (uint32_t)buff & ~31UL;
    uint32_t cache_end = ((uint32_t)buff + (uint32_t)Length + 31UL) & ~31UL;
